...
const exactRuleProfiles = {
  "ブリーク・ホスト": {
    verified: "2026年6月ルール更新反映",
    rules: [
      { timing: "常時", name: "病める者たち", effect: "味方モデルの攻撃でクリティカルヒットが出るたび、病ポイントを1点獲得（最大7点）。" },
      { timing: "第3ラウンド以降・自軍移動フェイズ", name: "悪魔の召喚", effect: "予備のプスゴイル・ブライトロード1体とプレイグベアラー1ユニットを、敵から6インチより遠くに配置できる。" },
      { timing: "任意のターン終了時", name: "ナーグルの抱擁", effect: "病ポイントを任意数消費。1点ごとに戦闘中の敵を1回選び、4+で1致命的ダメージ。同じ敵を複数回選択可。" },
      { timing: "自軍ヒーローフェイズ・各フェイズ1回", name: "豊穣の座", effect: "味方ユニット1つをD3回復。" },
      { timing: "常時", name: "驚異に満ちた肉体", effect: "味方モデルが撃破された時、除去前に1インチ以内の敵を選ぶ。4+で1致命的ダメージ。" },
      { timing: "自軍移動フェイズ", name: "ナーグルの庭師", effect: "将軍のみが争っている目標で3+を出すと汚染。味方はその目標を争う間ウォード4+。敵に支配されると解除。" },
      { timing: "自軍射撃フェイズ", name: "疫病の息", effect: "将軍から7インチ以内の敵を選び、そのユニットのモデル数だけダイス。5+ごとに1致命的ダメージ。" },
      { timing: "任意のターン終了時", name: "プレイグベアラー召喚者", effect: "将軍から完全に14インチ以内のプレイグベアラーに、撃破済みモデル1体を復帰。" }
    ]
  },
  "ルリッド・ドリーム": {
    verified: "2026年6月ルール更新反映",
    rules: [
      { timing: "各バトルラウンド開始時", name: "スラーネッシュの誘惑", effect: "誘惑ダイスを6個受け取る。敵の失敗したヒット・ウーンズ・セーヴに提示できる。同じ敵ユニットには各フェイズ1回まで。" },
      { timing: "誘惑を受諾した時", name: "受諾", effect: "失敗したロールを6に置換（再ロール・修正不可）。自分はD6堕落ポイントを得る。" },
      { timing: "誘惑を拒否した時", name: "拒否", effect: "そのロールを行ったユニットにD3致命的ダメージ。" },
      { timing: "各ラウンド終了時", name: "誘惑ダイス消失", effect: "残った誘惑ダイスはすべて失われる。" },
      { timing: "常時・12点以上", name: "魅惑的な苦痛", effect: "味方ユニットはラン後でも同じターンに突撃可能。" },
      { timing: "常時・18点以上", name: "嗜虐的な悪意", effect: "味方の近接武器にクリティカル（致命的）が付く。" },
      { timing: "常時・24点以上", name: "忘我の耽溺", effect: "味方ユニットはウォード5+。" },
      { timing: "任意の戦闘フェイズ・バトル中1回", name: "比類なき速度", effect: "味方ユニット1つに、そのフェイズ中先手効果。" },
      { timing: "自軍移動フェイズ", name: "陽動の焦点", effect: "味方ユニット1つは、そのターン中退却後でも射撃・突撃可能。" },
      { timing: "自軍射撃フェイズ・各ターン1回", name: "歪んだ鏡", effect: "将軍から9インチ以内の敵を選び4+。次の自分のターン開始までセーヴ-1。" }
    ]
  },
  "ソウルレイド・ハント": {
    verified: "2026年6月ルール更新反映",
    rules: [
      { timing: "第1ラウンド", name: "干潮", effect: "味方を対象とする射撃攻撃のヒットロール-1。" },
      { timing: "第2ラウンド", name: "満ち潮", effect: "味方はラン後でも同じターンに射撃・突撃可能。" },
      { timing: "第3ラウンド", name: "高潮", effect: "味方ユニットは先手効果。" },
      { timing: "第4ラウンド", name: "引き潮", effect: "味方は退却後でも同じターンに射撃・突撃可能。" },
      { timing: "常時", name: "砕ける波の技", effect: "同じターンに突撃したナマーティ・スロールの近接武器は貫通値+1。" },
      { timing: "任意の戦闘フェイズ・バトル中1回", name: "精神閃光", effect: "将軍と戦闘中の敵を選ぶ。そのフェイズ中、その敵の攻撃は修正前ヒット6のみ成功。" },
      { timing: "任意のターン終了時・軍につき各ターン1回", name: "エーテル海の捕食者", effect: "そのターンに戦闘した味方騎兵1つをD3回復。" },
      { timing: "自軍ヒーローフェイズ", name: "美味なる餌", effect: "将軍の接敵範囲内にいる味方騎兵をそれぞれ1回復。" },
      { timing: "常時", name: "鋼殻の鎧", effect: "将軍のセーヴロールに対する全修正を無視。" },
      { timing: "自軍ヒーローフェイズ", name: "忍び寄る霧の儀式", effect: "完全に12インチ以内の味方を選び3+。次の自分のターン開始まで、その味方を対象とする攻撃のウーンズ-1。" }
    ]
  },
  "サーガ・アックスバンド": {
    verified: "公式スピアヘッドPDF準拠",
    rules: [
      { timing: "各自軍ターン開始時・各ルーンはバトル中1回", name: "ルーン覚醒", effect: "未使用ルーンを1つ選びD6。1～5で通常効果、6で強化効果も追加。次の自分のターン開始まで持続。" },
      { timing: "ルーン", name: "憤怒のルーン", effect: "通常：近接攻撃のヒット+1。強化：さらに近接武器の攻撃回数+1。" },
      { timing: "ルーン", name: "灼熱のルーン", effect: "通常：近接武器の貫通値+1。強化：さらにダメージ+1。" },
      { timing: "ルーン", name: "燃える決意のルーン", effect: "通常：味方にウォード5+。強化：さらにセーヴ+1。" },
      { timing: "ルーン", name: "不断の熱意のルーン", effect: "通常：移動力+2インチ。強化：さらに突撃ロール+2。" },
      { timing: "第1ラウンド開始時・バトル中1回", name: "マグマ坑道", effect: "味方を最大2ユニット選び、敵から6インチより遠い任意の位置へ再配置。" },
      { timing: "自軍射撃フェイズ・軍につき各ターン1回", name: "ファイアスティール投げ斧", effect: "戦闘中でなく敵の10インチ以内にいる味方を任意数選ぶ。各ユニットごとに敵を選び4+でD3致命的ダメージ。" },
      { timing: "任意のターン開始時", name: "死ぬには頑固すぎる", effect: "将軍をD3回復。" },
      { timing: "自軍ヒーローフェイズ", name: "グリムニルの角笛", effect: "将軍が戦闘中でなければ使用。撃破済みモデルのいる味方ごとに3+で1体復帰。" },
      { timing: "任意のヒーローフェイズ・バトル中1回", name: "旗を打ち立てよ", effect: "そのターン中、バトルスミスの完全に12インチ以内の味方はウォード5+。" }
    ]
  }
};

function findExactRuleProfile(army) {
  const key = Object.keys(exactRuleProfiles).find(name => army.includes(name));
  return key ? exactRuleProfiles[key] : null;
}

function exactRuleMatchesNow(rule) {
  const timing = rule.timing;
  const phaseTitle = steps[state.phaseIndex]?.title || "";

  if (timing.includes(`第${state.round}ラウンド`)) return true;
  if (timing.includes("常時") || timing === "ルーン") return true;
  if (phaseTitle.includes("ラウンド開始") && (timing.includes("ラウンド開始") || timing.includes("ターン開始"))) return true;
  if (phaseTitle.includes("ヒーロー") && timing.includes("ヒーロー")) return true;
  if (phaseTitle.includes("移動") && timing.includes("移動")) return true;
  if (phaseTitle.includes("射撃") && timing.includes("射撃")) return true;
  if (phaseTitle.includes("突撃") && timing.includes("突撃")) return true;
  if (phaseTitle.includes("戦闘") && timing.includes("戦闘")) return true;
  if (phaseTitle.includes("ターン終了") && timing.includes("ターン終了")) return true;
  if (phaseTitle.includes("ラウンド終了") && timing.includes("ラウンド終了")) return true;
  return false;
}

function renderExactRules(content, army) {
  const profile = findExactRuleProfile(army);
  if (!profile) return;

  const section = document.createElement("section");
  section.className = "exact-rules-section";

  const head = document.createElement("div");
  head.className = "exact-rules-head";
  head.innerHTML = `
    <div>
      <span class="eyebrow">VERIFIED RULES</span>
      <h3>正規化済み軍ルール</h3>
    </div>
  ...
  list.className = "exact-rule-list";

  profile.rules.forEach(rule => {
    const card = document.createElement("article");
    card.className = `exact-rule-card ${exactRuleMatchesNow(rule) ? "is-current" : ""}`;
    card.innerHTML = `
      <div class="exact-rule-timing">${rule.timing}</div>
...
function loadState() {
  try {
    ...
    return {
      ...
      completedReminders: { ...(saved.completedReminders || {}) },
...
function renderArmyTools() {
  ...
  if (army.includes("ブリーク・ホスト")) {
    ...
    disease.innerHTML = "<h3>病ポイント</h3><p>ナーグルの抱擁で、消費した各ポイントにつき戦闘中の敵を1回選び、4+で1致命的ダメージ。</p>";
...
function isReinforcementReminder(text) {
  return /(増援|予備|配置済み|到着|復帰)/.test(text);
}

function getRoundScoreData(round = state.round) {
  if (!state.roundScores) state.roundScores = {};
  if (!state.roundScores[round]) {
    state.roundScores[round] = {
      player: 0,
      opponent: 0,
      applied: false
    };
  }
  return state.roundScores[round];
}

function renderRoundScoreCounter(container, side) {
  const data = getRoundScoreData();
  container.innerHTML = "";
  ...
  wrap.innerHTML = `
    <button type="button" data-action="minus">−</button>
    <strong>${Number(data[side] || 0)}</strong>
    <button type="button" data-action="plus">＋</button>
  `;

  wrap.querySelector('[data-action="minus"]').addEventListener("click", () => {
    if (data.applied) return;
    data[side] = Math.max(0, Number(data[side] || 0) - 1);
    saveState();
    renderRoundScorePanel();
  });

  wrap.querySelector('[data-action="plus"]').addEventListener("click", () => {
    if (data.applied) return;
    data[side] = Number(data[side] || 0) + 1;
    saveState();
    renderRoundScorePanel();
  });
...
function renderRoundScorePanel() {
  const data = getRoundScoreData();

  $("roundScoreLabel").textContent = `第${state.round}ラウンド`;
  $("roundPlayerName").textContent = state.playerName;
  $("roundOpponentName").textContent = state.opponentName;

  renderRoundScoreCounter($("roundPlayerScoreCounter"), "player");
  renderRoundScoreCounter($("roundOpponentScoreCounter"), "opponent");

  $("applyRoundScoreBtn").disabled = data.applied;
  $("resetRoundScoreBtn").disabled = data.applied;

  if (data.applied) {
    $("roundScoreStatus").textContent =
      `反映済み：${state.playerName} +${data.player}点／${state.opponentName} +${data.opponent}点`;
  } else {
    $("roundScoreStatus").textContent = "まだ合計へ反映していません。";
  }
}
...
function renderActionLog() {
  ...
  if (!state.actionLog?.length) {
    ...
    empty.textContent = "まだ処理履歴はありません。";
...
function reminderMatchesCurrentStep(text) {
  const phaseTitle = steps[state.phaseIndex]?.title || "";
  const phaseMap = [
    ["ラウンド開始", ["ラウンド開始", "各ラウンド", "今ラウンド"]],
    ["ヒーローフェイズ", ["ヒーローフェイズ", "呪文", "祈り"]],
    ["移動フェイズ", ["移動フェイズ", "移動", "配置", "予備", "増援", "再配置"]],
    ["射撃フェイズ", ["射撃フェイズ", "射撃"]],
    ["突撃フェイズ", ["突撃フェイズ", "突撃"]],
    ["戦闘フェイズ", ["戦闘フェイズ", "戦闘", "撃破"]],
    ["ターン終了", ["ターン終了", "戦闘後", "回復", "復帰"]],
    ["ラウンド終了", ["ラウンド終了", "次ラウンド"]]
  ];

  const found = phaseMap.find(([phase]) => phaseTitle.includes(phase));
  if (!found) return false;
  return found[1].some(word => text.includes(word));
}

function reminderMatchesRound(text) {
  const patterns = [
    { re: /第2ラウンド以降/, ok: state.round >= 2 },
    { re: /第3ラウンド以降/, ok: state.round >= 3 },
    { re: /第4ラウンド/, ok: state.round === 4 },
    { re: /第([1-4])ラウンド/, ok: null }
  ];

  for (const item of patterns) {
    if (item.re.test(text)) {
      if (item.ok !== null) return item.ok;
      const match = text.match(/第([1-4])ラウンド/);
      return match ? state.round === Number(match[1]) : false;
    }
  }
  return false;
}

function getLiveRemindersForSide(side) {
  const army = side === "player" ? state.playerArmy : state.opponentArmy;
  const profile = findArmyProfile(army);
  const results = [];
  const exactProfile = findExactRuleProfile(army);

  if (exactProfile?.rules) {
    exactProfile.rules
      .filter(exactRuleMatchesNow)
      .forEach(rule => {
        results.push({
          side,
          text: `${rule.name}：${rule.effect}`,
          roundAlert: rule.timing.includes("ラウンド") || rule.timing.includes("ターン開始")
        });
      });
  }

  if (!exactProfile && profile?.reminders) {
    profile.reminders.forEach(text => {
      const phaseMatch = reminderMatchesCurrentStep(text);
      const roundMatch = reminderMatchesRound(text);
      if (phaseMatch || roundMatch) {
        results.push({
          side,
          text,
          roundAlert: roundMatch
        });
      }
    });
  }

  // Special bespoke armies
  if (army.includes("ソウルレイド・ハント")) {
    const tideNames = {
      1: "第1ラウンド：干潮",
      2: "第2ラウンド：満ち潮",
    ...
    results.push({
      side,
      text: `現在の死の潮流：${tideNames[state.round]}`,
      roundAlert: true
    });
  }

  if (army.includes("ルリッド・ドリーム") && state.phaseIndex === 0) {
    results.push({
      side,
      text: "ラウンド開始時：誘惑ダイスを6個に補充",
      roundAlert: true
    });
  }

  if (army.includes("サーガ・アックスバンド") && steps[state.phaseIndex]?.title.includes("ラウンド開始")) {
    results.push({
      side,
      text: "未使用のウル＝ゴールド・ルーンを1つ選択",
      roundAlert: true
    });
  }

  if (army.includes("ブリーク・ホスト")) {
    const title = steps[state.phaseIndex]?.title || "";
    if (title.includes("ターン終了")) {
      results.push({
        side,
        text: "ターン終了時：病ポイント消費とモデル復帰を確認",
        roundAlert: false
      });
    }
  }
...
function renderBattleContext() {
  $("battlepackDisplay").textContent = state.battlepack;
  $("priorityDisplay").textContent = `${sideName(state.priority)}が先攻`;
  $("roundStatusDisplay").textContent =
    `第${state.round}ラウンド・${steps[state.phaseIndex]?.title || ""}`;
}

function renderLiveReminders() {
  const content = $("liveReminderContent");
  const reminders = [
    ...getLiveRemindersForSide("player"),
    ...getLiveRemindersForSide("opponent")
  ];

  const incomplete = reminders.filter(item => !state.completedReminders?.[reminderKey(item)]);
  $("liveReminderCount").textContent = `${incomplete.length}件未完了`;
  content.innerHTML = "";

  $("nextPhaseBtn").classList.toggle("next-warning", incomplete.length > 0);

  if (!reminders.length) {
    const empty = document.createElement("div");
    empty.className = "live-reminder-empty";
    empty.textContent = "このタイミングで自動表示する軍固有確認はありません。";
    content.appendChild(empty);
    return;
  }

  const list = document.createElement("div");
  list.className = "live-reminder-list";

  reminders.forEach(item => {
    const key = reminderKey(item);
    const completed = Boolean(state.completedReminders?.[key]);

    const row = document.createElement("div");
    row.className =
      `live-reminder-item ${item.side} ${item.roundAlert ? "round-alert" : ""} ${completed ? "completed" : ""} ${isReinforcementReminder(item.text) ? "reinforcement-alert" : ""}`;

    const side = document.createElement("div");
    side.className = "live-reminder-side";
    side.textContent = item.side === "player" ? "自分" : "相手";

    const main = document.createElement("div");
    main.className = "live-reminder-main";
    ...
    button.type = "button";
    button.className = "reminder-done-button";
    button.textContent = completed ? "未完了に戻す" : "完了";
    button.addEventListener("click", () => {
      if (!state.completedReminders) state.completedReminders = {};
      state.completedReminders[key] = !completed;

      if (!completed) {
        addActionLog(
          `第${state.round}ラウンド ${steps[state.phaseIndex].title}：${item.side === "player" ? state.playerName : state.opponentName}「${item.text}」を完了`
        );
      } else {
        saveState();
      }
...
function startBattle() {
  ...
  state.battlepack = state.desolationMode ? "City of Ash" : $("battlepack").value;
  state.started = true;
...
$("desolationMode").addEventListener("change", (event) => {
  if (event.target.checked) {
    $("battlepack").value = "City of Ash";
  }
});

$("battlepack").addEventListener("change", (event) => {
  if ($("desolationMode").checked && event.target.value !== "City of Ash") {
    event.target.value = "City of Ash";
  }
});
...
$("applyRoundScoreBtn").addEventListener("click", () => {
  const data = getRoundScoreData();
  if (data.applied) return;

  state.playerScore += Number(data.player || 0);
  state.opponentScore += Number(data.opponent || 0);
  data.applied = true;

  addActionLog(
    `第${state.round}ラウンド得点を反映：${state.playerName} +${data.player}点／${state.opponentName} +${data.opponent}点`
  );

  render();
});

$("resetRoundScoreBtn").addEventListener("click", () => {
  const data = getRoundScoreData();
  if (data.applied) return;
...
$("clearActionLogBtn").addEventListener("click", () => {
  state.actionLog = [];
  saveState();
  renderActionLog();
});
